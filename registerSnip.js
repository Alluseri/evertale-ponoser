...
async function CreateSession(UserID, CLID, Version, Device, Shard, Language, Region) {
	...
	return await GzipToJSON(await fetch("https://apialt.prd.evertaleserver.com/login", {
		...
		'User-Agent': 'UnityPlayer/2021.2.18f1 (UnityWebRequest/1.0, libcurl/7.80.0-DEV)',
		'UserAgent': 'UnityPlayer/2021.2.18f1 (UnityWebRequest/1.0, libcurl/7.80.0-DEV)',
		body: Object.keys(details).map(key => encodeURIComponent(key) + '=' + encodeURIComponent(details[key])).join('&')
	}));
}
...
(async () => {
	...
	if (!z) {
		console.log("IP banned, who cares tho?");
		return;
	}
	var m = await RestoreAccount(z.rcode, language, region);
	var v = (await CreateSession(z.uid, m.recover.clid, "2.0.67", device, shard, language, region)).login.sesid;
	await UpdateSettings(v, "defnotneko", "who am i");
	console.log("[WC] Created account at " + shard + ": " + z.rcode + "; " + z.uid + "; " + m.recover.clid + "; " + v);
	let MyChat = new EvertaleChat(v, z.uid, region);
	await MyChat.Awaitable;
	MyChat.SendMessage("Arch Linux supremacy");
	console.log("Emitted EC message");
})();